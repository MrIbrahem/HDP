...
- **Throttle convention:** 0.3s after each *uncached* network fetch in the editcounts loop (gated by `was_cached`); 0.1s inside `get_home_wikis_and_registration`. Preserve when refactoring.
...
- **Effort**: S.
- **Risk**: MED. The fix is small, but the correct shape depends on how the live `User:Mr. Ibrahem/hdp` page is actually structured (does it contain a section titled `"Open requests"`? or is the intent to feed a *category* name through a category-mapping branch that doesn't exist?). Two viable fix shapes; needs maintainer confirmation (see Direction #1).
...
- **Evidence**:
  ...
  - `src/api/mwclient_req.py:287-294` — loop over users calling `get_global_userinfo` one at a time; `meta=globaluserinfo` only accepts a single username so this cannot be batched but *can* be parallelized.
